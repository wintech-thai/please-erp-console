...
function InviteConfirmContent() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setInviteInfo({ userName: parsed.UserName || parsed.username || '', email: parsed.Email || parsed.email || '', orgUserId: parsed.OrgUserId || parsed.orgUserId, invitedBy: parsed.InvitedBy || parsed.invitedBy })
    } catch {
      try {
  ...
  const handleAccept = async () => {
    ...
    setSubmitting(true)
    try {
      await userApi.confirmExistingUserInvite(orgId, token, { username: inviteInfo.userName, email: inviteInfo.email, orgUserId: inviteInfo.orgUserId })
              ...
              {errorDecode ? (
                ...
                <div className="space-y-5">
                  ...
                  {inviteInfo.invitedBy && (
                    <p className="text-xs text-center text-gray-400">{ti.invitedBy} <span className="font-medium text-gray-600">{inviteInfo.invitedBy}</span></p>
                  )}
                  <button onClick={handleAccept} disabled={submitting} className="w-full flex items-center justify-center gap-2 py-3 text-sm font-semibold text-white bg-primary-600 hover:bg-primary-700 disabled:opacity-60 rounded-xl transition-colors shadow-sm">
                    {submitting ? <><Loader2 className="w-4 h-4 animate-spin" />{ti.confirmingButton}</> : <><CheckCircle2 className="w-4 h-4" />{ti.acceptButton}</>}
                  </button>
