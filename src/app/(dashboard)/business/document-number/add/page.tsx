...
export default function AddDocumentNumberPage() {
  ...
  const [docType, setDocType] = useState<string>('InventoryImport')
  const [format, setFormat] = useState('IMP-${yyyy}-${mm}-${seq}')
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!docType || !format) { toast.error(dn.requiredFields); return }
    setSaving(true)
    try {
      const pendingTag = tagInput.trim()
      const finalTags = pendingTag && !tags.includes(pendingTag) ? [...tags, pendingTag] : tags
      await documentNumberApi.addConfig({ documentType: docType, documentFormat: format, seqDigit, resetType, yearOffset, tags: finalTags.join(',') })
      toast.success(dn.addSuccess)
      setIsDirty(false)
      router.push('/business/document-number')
    } catch (err: unknown) {
      const msg = (err as { message?: string })?.message
      toast.error(msg || dn.addError)
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="flex flex-col overflow-hidden h-[calc(100dvh-5rem)] sm:h-[calc(100dvh-6.5rem)]">
      ...
      <div className="flex-none flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{dn.addTitle}</h1>
          <p className="text-base text-gray-500 mt-0.5">{t.nav.documentNumber}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0 overflow-hidden">
        <div className="flex-1 overflow-y-auto flex flex-col gap-4 pb-2">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 px-7 py-6">
            <SectionHeader>{dn.addTitle}</SectionHeader>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 px-7 py-6">
            ...
            <FormField label={dn.fieldTags}>
              <div
                ...
                onClick={() => document.getElementById('doc-number-tag-input')?.focus()}
              >
                {tags.map(tag => (
                  <span key={tag} className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium bg-primary-50 text-primary-700 rounded-full">
                    {tag}
                    <button type="button" onClick={() => { setTags(p => p.filter(t => t !== tag)); markDirty() }}>
                      <X className="w-3 h-3 text-primary-400 hover:text-primary-700" />
                    </button>
                  </span>
                ))}
                <input
                  id="doc-number-tag-input"
                  value={tagInput}
