...
const SS_KEY = 'inventory_item_highlight'
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onCancel}>
      ...
      >
        ...
        <h3 className="text-lg font-bold text-white mb-1">{ii.deleteTitle}</h3>
        {name && <p className="text-sm font-semibold text-white/90 mb-2">&ldquo;{name}&rdquo;</p>}
        <p className="text-sm text-white/60 mb-7">{ii.deleteDesc} {ii.deleteCannotUndo}</p>
        <div className="flex gap-3">
          ...
          <button onClick={onConfirm} disabled={deleting} className="flex-1 py-2.5 text-sm font-semibold text-white bg-red-600/80 rounded-xl hover:bg-red-600 disabled:opacity-60 transition-colors uppercase">
            {deleting ? ii.deleting : ii.deleteBtn}
          </button>
        </div>
      </div>
    </div>
...
function InventoryItemContent() {
  ...
  const [items, setItems] = useState<InventoryItemItem[]>([])
  ...
  const fetchItems = useCallback(async (search = searchTerm, type = typeFilter, p = page, perPage = itemsPerPage) => {
    setLoading(true)
    try {
      const offset = (p - 1) * perPage + 1
      const [listRes, countRes] = await Promise.allSettled([
        inventoryItemApi.getItems({ fullTextSearch: search || undefined, itemType: type || undefined, offset, limit: perPage }),
        inventoryItemApi.getItemCount({ fullTextSearch: search || undefined, itemType: type || undefined }),
      ])
      if (listRes.status === 'fulfilled') setItems(Array.isArray(listRes.value.data) ? listRes.value.data : [])
      if (countRes.status === 'fulfilled') { const raw = countRes.value.data as unknown; setTotal(typeof raw === 'number' ? raw : 0) }
    } catch { toast.error(ii.loadFailed) } finally { setLoading(false) }
  }, [ii.loadFailed, searchTerm, typeFilter, page, itemsPerPage])
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      document.getElementById(`inventory-item-row-${highlightIdParam}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }, 300)
  ...
  function handleSearch() { setPage(1); fetchItems(searchTerm, typeFilter, 1) }

  function handleTypeFilterChange(value: string) {
    setTypeFilter(value)
    setPage(1)
    fetchItems(searchTerm, value, 1)
  }
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    try {
      await inventoryItemApi.deleteItemById(deleteModal.id)
      toast.success(ii.deletedSuccess)
      setDeleteModal({ open: false }); setCheckedId(null)
      fetchItems()
    } catch { toast.error(ii.failedToDelete) } finally { setDeleting(false) }
  }
  ...
  return (
    <div className="flex flex-col overflow-hidden h-[calc(100dvh-5rem)] sm:h-[calc(100dvh-6.5rem)]">
      ...
      <div className="flex-none flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold text-gray-900">{ii.title}</h1>
        <button
          onClick={() => router.push('/business/inventory/items/add')}
      ...
      <div className="flex-none flex flex-wrap items-center gap-3 mb-4">
        <div className="flex items-center gap-2 flex-1 min-w-56 max-w-xs bg-white border border-gray-200 rounded-lg px-3 py-2 shadow-sm">
          ...
          <input
            ...
            placeholder={ii.searchPlaceholder}
            className="flex-1 text-sm bg-transparent outline-none text-gray-700 placeholder-gray-400"
          />
          {searchTerm && (
            <button onClick={() => { setSearchTerm(''); setPage(1); fetchItems('', typeFilter, 1) }} className="text-gray-400 hover:text-gray-600">✕</button>
          )}
        </div>
        <select
          value={typeFilter}
        ...
        >
          <option value="">{ii.allTypes}</option>
        ...
        <button
          onClick={() => { const item = items.find(r => r.id === checkedId); setDeleteModal({ open: true, id: checkedId ?? undefined, name: item?.code }) }}
          disabled={!checkedId}
          ...
          <table className="min-w-full text-sm">
            <thead className="sticky top-0 z-10">
              <tr className="bg-gray-50/70 border-b border-gray-100">
                ...
                <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{ii.colCode}</th>
                <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{ii.colName}</th>
                <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{ii.colUnit}</th>
                <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{ii.colGroup}</th>
                <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{ii.colType}</th>
                <th className="w-28 px-4 py-3.5 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">{ii.colAction}</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="py-16 text-center">
                  ...
                  <p className="text-sm text-gray-400">{ii.noData}</p>
                </td></tr>
              ) : items.map((item, idx) => {
                const highlighted = selectedRowId === item.id
                const isChecked = checkedId === item.id
                const typeLabel = itemTypes.find(o => o.code === item.itemType)?.description || item.itemType
                return (
                  <tr
                    key={item.id}
                    id={`inventory-item-row-${item.id}`}
                  ...
                  >
                    <td className="px-6 py-4" onClick={e => e.stopPropagation()}>
                      <input
                        type="checkbox"
                        checked={isChecked}
                        onChange={() => setCheckedId(prev => prev === item.id ? null : item.id)}
                        className="rounded border-gray-300 text-primary-600 focus:ring-primary-500 w-4 h-4 cursor-pointer"
                      />
                    </td>
                    <td className="px-6 py-4" onClick={e => e.stopPropagation()}>
                      <button
                        onClick={() => router.push(`/business/inventory/items/${item.id}/update`)}
                    ...
                    <td className="px-6 py-4 text-gray-600 max-w-xs">
                      <span className="line-clamp-1">{item.nameTh || '—'}</span>
                    </td>
                    <td className="px-6 py-4 text-gray-600">{item.unit || '—'}</td>
                    <td className="px-6 py-4 text-gray-600">{item.itemGroup || '—'}</td>
