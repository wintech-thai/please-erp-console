...
function ApiKeysContent() {
  ...
  const SS_KEY = 'admin_api_keys_highlight'
  ...
  const fetchKeys = async (currentPage: number, keyword: string = '') => {
    ...
    try {
      ...
      const data: ApiKeyItem[] = Array.isArray(raw) ? raw : (raw?.apiKeys ?? [])
      setKeys(data)
  ...
  useEffect(() => {
    ...
    if (keys.length > 0) selectRow(keys[0].keyId)
          ...
          <table className="min-w-full">
            <thead>
              <tr className="bg-gray-50/70 border-b border-gray-100">
                ...
                <th className="w-14 px-4 py-3.5 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">{t.apiKeys.colAction}</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
