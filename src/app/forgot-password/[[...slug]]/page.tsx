...
function ForgotPasswordContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      await userApi.confirmForgotPassword(orgId, token, { username: userInfo.userName, email: userInfo.email, password, orgUserId: userInfo.orgUserId })
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="px-8 py-6 space-y-5">
              ...
              <button type="submit" disabled={submitting} className="w-full flex items-center justify-center gap-2 py-3 text-sm font-semibold text-white bg-primary-600 hover:bg-primary-700 disabled:opacity-60 rounded-xl transition-colors shadow-sm">
                {submitting ? <><Loader2 className="w-4 h-4 animate-spin" />{fp.processing}</> : fp.submit}
              </button>
